...
public class Routes {
    ...
    private static void login(Context ctx) {
        ...
        if (username == null || password == null) {
            ctx.status(400).json(err("Campos requeridos"));
            return;
        }
    ...
    private static void registerUser(Context ctx) {
        ...
        if (username == null || email == null || password == null) {
            ctx.status(400).json(err("Todos los campos son requeridos"));
            return;
        }
        if (password.length() < 6) {
            ctx.status(400).json(err("La contraseña debe tener mínimo 6 caracteres"));
            return;
        }
        try {
    ...
    private static void me(Context ctx) {
        ...
        if (userId == null)
            throw new UnauthorizedResponse("No has iniciado sesión");
    ...
    private static void publishEvent(Context ctx) {
        requireOrganizerOrAdmin(ctx);
        Long userId = ctx.sessionAttribute("userId");
        ctx.json(eventMap(EventService.setStatus(Long.parseLong(ctx.pathParam("id")), EventStatus.PUBLISHED, userId),
                userId));
    }
    ...
    private static void unpublishEvent(Context ctx) {
        requireOrganizerOrAdmin(ctx);
        Long userId = ctx.sessionAttribute("userId");
        ctx.json(eventMap(EventService.setStatus(Long.parseLong(ctx.pathParam("id")), EventStatus.DRAFT, userId),
                userId));
    }
    ...
    private static void cancelEvent(Context ctx) {
        requireOrganizerOrAdmin(ctx);
        Long userId = ctx.sessionAttribute("userId");
        ctx.json(eventMap(EventService.setStatus(Long.parseLong(ctx.pathParam("id")), EventStatus.CANCELLED, userId),
                userId));
    }
    ...
    private static void scanQR(Context ctx) {
        ...
        if (qrContent == null || qrContent.isBlank()) {
            ctx.status(400).json(err("Contenido QR requerido"));
            return;
        }
        String token = QRService.extractToken(qrContent);
        if (token == null) {
            ctx.status(400).json(err("Formato QR inválido"));
            return;
        }
        try {
    ...
    private static void adminBlockUser(Context ctx) {
        requireAdmin(ctx);
        try {
            ctx.json(userMap(UserService.setBlocked(Long.parseLong(ctx.pathParam("id")), true)));
        } catch (IllegalArgumentException e) {
            ctx.status(400).json(err(e.getMessage()));
        }
    }
    ...
    private static void adminUnblockUser(Context ctx) {
        requireAdmin(ctx);
        try {
            ctx.json(userMap(UserService.setBlocked(Long.parseLong(ctx.pathParam("id")), false)));
        } catch (IllegalArgumentException e) {
            ctx.status(400).json(err(e.getMessage()));
        }
    }
    ...
    private static void requireOrganizerOrAdmin(Context ctx) {
        ...
        if (user.getRole() == Role.PARTICIPANT)
            throw new ForbiddenResponse("Se requiere rol Organizador o Administrador");
    }
    ...
    private static void requireAdmin(Context ctx) {
        ...
        if (user.getRole() != Role.ADMIN)
            throw new ForbiddenResponse("Solo los administradores pueden hacer esto");
    }
