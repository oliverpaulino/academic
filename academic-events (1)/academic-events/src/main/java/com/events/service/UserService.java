...
public class UserService {
    ...
    public static void initAdminUser() {
        if (repo.findByUsername("admin").isEmpty()) {
            User admin = User.builder()
                    ...
                    .build();
            repo.save(admin);
            log.info("Admin creado → usuario: admin | contraseña: admin");
        }
    }
    ...
    public static User setBlocked(Long userId, boolean blocked) {
        ...
        if (!u.isDeletable())
            throw new IllegalArgumentException("No se puede modificar el usuario del sistema");
    ...
    public static User setRole(Long userId, Role role) {
        User u = repo.findById(userId).orElseThrow(() -> new IllegalArgumentException("Usuario no encontrado"));
        if (!u.isDeletable())
            throw new IllegalArgumentException("No se puede cambiar el rol del usuario del sistema");
        u.setRole(role);
        return repo.update(u);
    }
}
